...
const CustomForm = ({ item, onClose }: { item?: any, onClose: () => void }) => {
    ...
    const onSubmit = useCallback(() => {
        ...
        fn(url, {
            onBefore: () => {
                ...
                // setData('owner_id', data.owner_id?.trim())
                // setData('avatar_url', data.avatar_url?.trim())
            ...
            onError: () => {
                ...
                // @todo: call toast or something like that, whats sonner?
            },
        })
    }, [item, data, setData])
    ...
    return <SimpleModal onClose={onClose} title="Create Team" description="Create a new team" isLoading={processing}><>
        ...
        <ShadSelect required label="Owner ID" name="owner_id" value={data.owner_id} onChange={(e: string) => setData('owner_id', e)} list={users} />
