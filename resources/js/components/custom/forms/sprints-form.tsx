...
import type { TSprintStatus } from '@/types/models/sprint';
...
const CustomForm = ({ item, onClose }: { item?: any; onClose: () => void }) => {
    ...
    const fetchSelectorsData = useCallback(async () => {
        ...
        await Promise.all([get(project.all().url, setProjects)]).finally(() => {
    ...
    const statuses = useMemo(() => {
        return Object.keys(SprintStatusEnum).map((i: string) => ({
            value: i,
    ...
    const onSubmit = useCallback(() => {
        ...
        fn(url, {
            ...
            onError: () => {
                toast.error(`Sprint creation failed: ${errors.toString()}`, {
    ...
    return (
        <SimpleModal
            ...
            isLoading={processing || projectsLoading}
        >
            <>
                <ShadInput
                    required
                ...
                <ShadSelect
                    ...
                    name="status"
                    value={data.status}
                    onChange={(e: string) => setData('status', e)}
                    list={statuses}
                    errors={errors}
                />
                ...
                <div className="flex justify-between gap-3">
                    <ShadInput
                        ...
                        value={data.velocity}
                        onChange={(e) =>
                            setData('velocity', parseNumber(e.target.value))
                        }
                        errors={errors}
                    />
                    <ShadInput
